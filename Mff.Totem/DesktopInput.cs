...
namespace Mff.Totem
{
	class DesktopInput : Input
	{
		...
		public override PointerInput GetPointerInput(byte id)
		{
			switch (id)
			{
				case 0:
		...
		public override List<PointerInput> GetPointerInputs()
		{
			return new List<PointerInput>() { LMB, RMB, MMB };
		}

		public override List<PointerInput> GetPointerInputsOutsideGui()
		{
			return inGui ? new List<PointerInput>() { } : GetPointerInputs();
		}

		public override bool GetInput(Inputs i, InputState state)
		{
			switch (i)
			{
				case Inputs.Pause:
